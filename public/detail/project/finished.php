...
if (isset($_POST['id_project']) && isset($_POST['id_user'])) {
    ...
    $resultChangesProjectStatus = mysqli_query($con, "UPDATE project SET project_status = 'Finished' WHERE id_project = '$id_project'");
    if ($resultChangesProjectStatus) {
        $resultChangesBidStatusFinished = mysqli_query($con, "UPDATE bid SET bid_status = 'Finished' WHERE id_project = '$id_project' AND id_worker = '$id_user'");
        if ($resultChangesBidStatusFinished) {
